...
export const AdminExamesVincular = () => {
  const { adminActiveUser, adminActiveExame } = adminStore((state) => state);
  const { addExame } = exameStore((state) => state);
  ...
  const enviarWebhook = async () => {
    ...
    const mensagemWebhook =
      ...
      `:white_check_mark: Registrado no Exame: **${adminActiveExame?.nome}** para o colaborador **${adminActiveUser?.nome}**\n\n` +
  ...
  const handleEnrollClick = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    e.stopPropagation();
    await addExame(adminActiveExame!, adminActiveUser!, input);
    await loadUser();
    await enviarWebhook(); // Chame a função para enviar o webhook após adicionar o Exame
  };
  ...
  return (
    <StyledContainer>
      {adminActiveExame ? (
        adminActiveUser ? (
          <p>
            Registrar colaborador {adminActiveUser?.nome}? ao Exame:{" "}
      ...
      {adminActiveUser && adminActiveExame && (
        <StyledSubmitButton
          $error={false}
          onClick={() => {
            setEnrollModal(true);
      ...
      {enrollModal && (
        <StyledModal>
          ...
          <div>
            <span>Vincule Exame : {adminActiveExame?.nome} - </span>
